...
class ItemPicturesSpec extends Specification {
  ...
  "ItemPicture" should {
    "If specified pictures is not found, 'notfound.jpg' will be returned." in {
    ...
    "If specified pictures is found and modified, it will be returned." in {
      val app = FakeApplication(additionalConfiguration = inMemoryDatabase() ++ withTempDir)
      running(TestServer(3333, app), Helpers.HTMLUNIT) { browser => DB.withConnection { implicit conn =>
    ...
    "If specified pictures is found but not modified, 304 will be returned." in {
      val app = FakeApplication(additionalConfiguration = inMemoryDatabase() ++ withTempDir)
      running(TestServer(3333, app), Helpers.HTMLUNIT) { browser => DB.withConnection { implicit conn =>
        val file = dir.resolve("2_1.jpg")
        Files.write(file, util.Arrays.asList("Hello"), Charset.forName("US-ASCII"))
