...
//        browser.find("#shippingDateTextBox_field"
    ...
    "Transaction type 'paypal express checkout' is shown" in {
      ...
      running(TestServer(3333, app), Helpers.FIREFOX) { browser => DB.withConnection { implicit conn =>
        ...
        browser.await().atMost(5, TimeUnit.SECONDS).until(".site").areDisplayed()
        browser.await().atMost(5, TimeUnit.SECONDS).until("td.status.t1000 .blockOverlay").isNotPresent()
        browser.await().atMost(5, TimeUnit.SECONDS).until("td.status.t1001 .blockOverlay").isNotPresent()
