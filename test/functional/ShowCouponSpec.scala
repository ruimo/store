  ...
  "ShowCoupon" should {
    "Can show coupon." in {
      ...
      running(TestServer(3333, app), Helpers.HTMLUNIT) { browser => DB.withConnection { implicit conn =>
        doRetry(classOf[java.lang.InterruptedException]) {
          ...
          browser.goTo(
            "http://localhost:3333" + 
            controllers.routes.CouponHistory.showInstantCoupon(site.id.get, item.id.get.id).url.addParm("lang", lang.code)
          )
