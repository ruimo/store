...
case class CreateSiteOwner(
  ...
  def save(implicit tokenGenerator: TokenGenerator, conn: Connection): (StoreUser, SiteUser) = {
    ...
    val storeUser = StoreUser.create(userName, firstName, middleName, lastName, email, hash, salt, UserRole.NORMAL)
...
object CreateSiteOwner {
  def fromForm(
    siteId: Long, userName: String, firstName: String, middleName: Option[String], lastName: String,
    email: String, passwords: (String, String)
  ): CreateSiteOwner =
    CreateSiteOwner(siteId, userName, firstName, middleName, lastName, email, passwords._1)

  def toForm(m: CreateSiteOwner) = Some(
    m.siteId, m.userName, m.firstName, m.middleName, m.lastName, m.email, (m.password, m.password)
  )
}
